...
$(document).ready(function(){
	...
	function preloadimages(){
		for (i=0;i<preloadimages.arguments.length;i++){
			...
			myimages[i].src='css/img/'+preloadimages.arguments[i];
		}
	};
	var template;
	var detailtemplate;
	...
	function updatejson(){	//add main content
		...
		if($(".td-normal-click").text())
			data1.time=caltime($(".td-normal-click").text());
	...
	function changestyle(json,i){	//change color style
			...
			var cat=style1.cat;
				if(json!=""){	//改变色系
					...
					$('.main-content-each:eq('+i+') .tag').css("background-image","url('css/img/tag-"+cat+".png')");
					$('.main-content-each:eq('+i+') .love-icon').hover(function(){
						if(!$(this).hasClass("love-select"))
						$(this).css("background-image","url('css/img/love-"+cat+".png')");				
					});
					$('.main-content-each:eq('+i+') .love-icon').mouseleave(function(){
						if(!$(this).hasClass("love-select")){
							$(this).css("background-image","url('css/img/love.png')")
						}
					});
					$('.main-content-each:eq('+i+') .love-icon').click(function(){
						if(!$(this).hasClass("love-select")){
							$(this).addClass("love-select");
							$(this).css("background-image","url('css/img/love-"+cat+"-selected.png')");
					...
					$('.main-content-each:eq('+i+') .pin-icon').hover(function(){
						...
						$(this).css("background-image","url('css/img/pin-"+cat+".png')")	
					});	
					$('.main-content-each:eq('+i+') .pin-icon').mouseleave(function(){
						if(!$(this).hasClass("pin-select"))
						$(this).css("background-image","url('css/img/pin.png')")
						
					});
					$('.main-content-each:eq('+i+') .pin-icon').click(function(){
						if(!$(this).hasClass("pin-select")){
							$(this).addClass("pin-select");
							$(this).css("background-image","url('css/img/pin-"+cat+"-selected.png')");
							num=Number($(this).next().text())+1;
							$(this).next().text(num);
							timechoose();
						}
					...
					if(json.ViewFlag=='1'){
						...
						$(".days-left .review").css({
							"background-image":"url('css/img/review-"+cat+".png')",
							"width":"115px",
							"height":"62px",
							"margin-left":"60px"
						});
	...
	function addnum(json){
		if(json.all!=""){
			...
			$(".purple").prepend("<div class='icon'><img  src='/css/img/icon-color.png' alt='' /></div>");

		}
		if(json.lecture!=""){
			$(".red").prepend("<p class='num'>"+json.lecture+"</p>");
			$(".red").prepend("<div class='icon'><img  src='/css/img/icon.png' alt='' /></div>");
	...
	function addaddition(json,i){ //add addition content
		if((json).hot=='1'){
			$('.main-content-each:eq('+i+') .main-content-img').prepend("<div class='hot'>HOT</div><img class='hot-corner' src='css/img/corner.png' />");
	...
	function bind(){//一些状态变化
		...
		$('#top').hover(function(){
			...
			if(num!=-1)
				num=(num+1)/2;
			if(num==-1||num==1/2)
				$('#top').css("background","url('css/img/top-all.png')");
			if(num==1)
				$('#top').css("background","url('css/img/top-lec.png')");
			if(num==2)
				$('#top').css("background","url('css/img/top-per.png')");
			if(num==3)
				$('#top').css("background","url('css/img/top-out.png')");
			if(num==4)
				$('#top').css("background","url('css/img/top-con.png')");
			if(num==5)
				$('#top').css("background","url('css/img/top-onl.png')");
			if(num==6)
				$('#top').css("background","url('css/img/top-oth.png')");
		});
		$('#top').click(function(){
		...
		$('#top').mouseleave(function(){
			$('#top').css("background","url('css/img/top.png')");
		});
	...
	var changedetailstyle = function(json){
				...
				if(json!=""){	//改变色系
					...
					$('#top').hover(function(){
						$('#top').css("background","url('css/img/top-"+cat+".png')");
	...
	function picstyle(content){	//报错消息
		cover(content);
		$("#join-time-content").css("background-image","url('/css/img/face.png')");
